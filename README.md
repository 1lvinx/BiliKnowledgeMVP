...
哔知后续不会停留在“B 站收藏整理工具”，而是会按 V2 蓝图逐步升级为 **AI 视频知识工作台**：让收藏视频、字幕、笔记、标签、开源项目和个人想法之间形成可搜索、可连接、可复用的知识系统。

| 阶段 | 目标 | 重点能力 |
|---|---|---|
| Phase 1 | MVP 稳定化 | 收藏同步、视频解析、Markdown 管理、SQLite 元数据、全局搜索 |
| Phase 2 | AI 知识提炼 | AI 摘要、AI 标签、分类、关键词、知识卡片、重复内容检测 |
| Phase 3 | 知识关系网络 | 本地 Embedding、语义搜索、知识图谱、视频/标签/项目/笔记关系连接 |
| Phase 4 | 个人知识 Agent | RAG、Agent 问答、围绕本地视频知识库进行追问和复用 |
| Phase 5 | 扩展生态 | 插件系统、可选同步、更多知识源接入和商业化探索 |

近期优先级会保持克制：先把导入稳定性、字幕链路、搜索命中率、笔记质量和 Doctor 诊断做好；暂不优先投入团队协作、复杂权限系统和社交功能。
...
- 完善 Token 费用估算和多模型配置；
- 补齐 macOS 签名、公证、Windows 签名和正式公开发布流程。
